...
def gconnect():
	...
	login_session['provider'] = 'google'  #not in Steve's
...
def getUserID(email):
    try:
        ...
        return user.id 
    except Exception:  #Steve's (mine just except)
        return None
...
def gdisconnect():
	...
	url = 'https://accounts.google.com/o/oauth2/revoke?token=%s' % credentials  # or access_token
	h = httplib2.Http()
...
"""Beginning of project body - CRUD fx"""
...
def newRestaurant():
    ...
    if request.method == 'POST':
        ...
        user_id = getUserID(login_session['email'])
        newRestaurant = Restaurant(name = request.form['name'], user_id=user_id)
